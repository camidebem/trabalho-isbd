...
/* Recupera o nome dos cozinheiros cujo salário é maior que todos os garçons. */
SELECT Pessoa.Nome
FROM restaurante.Pessoa
JOIN restaurante.Cozinheiro ON Pessoa.CPF = Cozinheiro.CPF
WHERE .Salario > ALL (
    SELECT Salario
    FROM restaurante.Funcionario
    WHERE CPF IN (SELECT CPF FROM restaurante.Garcom)
);
...
/* retorna todos os pedidos cujo valor é maior que todos os pedidos feitos pelo cliente com CPF '12345678901' */
SELECT *
FROM restaurante.Pedido
